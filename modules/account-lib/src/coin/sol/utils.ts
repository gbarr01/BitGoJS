...
import { MEMO_PROGRAM_PK, ValidInstructionTypesEnum, VALID_SYSTEM_INSTRUCTION_TYPES } from './constants';
import { TransactionType } from '../baseCoin';
import { NotSupported, UtilsError } from '../baseCoin/errors';
...
export function isValidRawTransaction(rawTransaction: string): boolean {
  try {
    // TODO add custom validation since it will parse wrongly and not fail on short base64 string
    SolTransaction.from(Buffer.from(rawTransaction, 'base64'));
    return true;
  } catch (e) {
    return false;
  }
...
export function getTransactionType(transaction: SolTransaction): TransactionType {
  ...
  if (
    ...
    getInstructionType(instructions[0]) === ValidInstructionTypesEnum.Create &&
    getInstructionType(instructions[1]) === ValidInstructionTypesEnum.InitializeNonceAccount
  ) {
